...
class Visualizer:
    ...
    def visualize(self, output, batch):
        ...
        depth_map = output['depth_map'][0].detach().cpu().numpy()
        img_depth_map = img_pred.copy()[:,:,0]
        img_depth_map[mask_at_box] = depth_map
        ...
        min_val = img_depth_map.max() - 1.5
        max_val = img_depth_map.max() - 0.3
        normalized_img_depth_map =  255 *(img_depth_map - min_val)/ (max_val-min_val)
        # normalized_img_depth_map = cv2.equalizeHist(normalized_img_depth_map.astype(np.uint8))
        cv2.imwrite(os.path.join(img_root, '{:04d}_depth.png'.format(index)),
                    normalized_img_depth_map)
