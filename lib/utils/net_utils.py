...
def transform_spconv1_spconv2(pretrained_model):
    for key in pretrained_model['net'].keys():
        if 'xyzc_net' in key and 'weight' in key and pretrained_model['net'][
                key].dim() == 5:
            # print(key)
            # print(pretrained_model['net'][key].shape)
            pretrained_model['net'][key] = pretrained_model['net'][
                key].permute([4, 0, 1, 2, 3])

    for key in pretrained_model['optim']['state'].keys():
        if pretrained_model['optim']['state'][key]['exp_avg'].dim() == 5:
            pretrained_model['optim']['state'][key][
                'exp_avg'] = pretrained_model['optim']['state'][key][
                    'exp_avg'].permute([4, 0, 1, 2, 3])
            pretrained_model['optim']['state'][key][
                'exp_avg_sq'] = pretrained_model['optim']['state'][key][
                    'exp_avg_sq'].permute([4, 0, 1, 2, 3])
...
def load_model(net,
    ...
    try:
        ...
        print('transform spconv 1 to spconv 2')
